	...
	public void Initialize(IncrementalGeneratorInitializationContext context)
	{
		context.RegisterPostInitializationOutput(static ctx =>
		{
			...
			ctx.AddSource("IOrmModel.g.cs",
				...
				{
					public static abstract List<TSelf> GetFromQuery(DbCommand command);
					public static abstract Task<List<TSelf>> GetFromQueryAsync(DbCommand command, CancellationToken token = default);
				}
				""");
			ctx.AddSource("DbCommandExtensions.g.cs",
				...
				{
					public static List<T> GetListOf<T>(this DbCommand command) where T: OrmGenerator.IOrmModel<T> => T.GetFromQuery(command);
					public static async Task<List<T>> GetListOfAsync<T>(this DbCommand command, CancellationToken token = default) where T: OrmGenerator.IOrmModel<T> => await T.GetFromQueryAsync(command, token);
				}
				""");
		});
		...
		context.RegisterSourceOutput(provider, (spc, model) =>
		{
			...
			{
				public static List<{{model.Name}}> GetFromQuery(DbCommand command)
				{
			...
			$$"""
				...
				public static async Task<List<{{model.Name}}>> GetFromQueryAsync(DbCommand command, CancellationToken token = default)
				{
					List<{{model.Name}}> result = [];
